...
export const getAllFoodItems = async (request, h) => {
  try {
    ...
    const foodItemsQuery = FoodItem.find(query).sort({ nama: 1 }).skip(skip).limit(limit);

    const totalItemsQuery = FoodItem.countDocuments(query);

    const [foodItems, totalItems] = await Promise.all([foodItemsQuery.exec(), totalItemsQuery.exec()]);
