...
export const getPendingFoodItems = async (request, h) => {
    try {
        ...
        const totalPendingItemsQuery = PendingFoodItem.countDocuments({ status: 'pending' });
...
export const approvePendingFoodItem = async (request, h) => {
    try {
        ...
        if (existingFoodItem) {
            ...
            throw Boom.conflict(`Makanan dengan nama '${foodData.nama}' sudah ada di daftar utama. Pengajuan ini ditolak.`);
        }
