  ...
  register: async (server) => {
    server.ext('onPreResponse', (request, h) => {
      ...
      if (response.isBoom) {
        ...
        let customMessage = response.message || response.output.payload.message;
        let status = 'fail';
        ...
        switch (statusCode) {
          case 400:
            ...
            break;
          case 401:
            customMessage = customMessage || 'Token tidak valid, kedaluwarsa, atau Anda belum login.';
            h.unstate('jwt', { path: '/', isSecure: process.env.NODE_ENV === 'production', isSameSite: 'Lax' });
            break;
          case 403:
            customMessage = customMessage || 'Anda tidak memiliki izin untuk mengakses sumber daya ini.';
            break;
          case 404:
            customMessage = customMessage || 'Halaman atau sumber daya yang diminta tidak ditemukan.';
            break;
          case 409:
            customMessage = response.output.payload.message || 'Terjadi konflik dengan data yang sudah ada.';
            break;
          case 413:
            customMessage = 'Ukuran file profil tidak boleh melebihi 5MB.';
            break;
          case 500:
      ...
      const isLogout = request.path === '/api/logout';
